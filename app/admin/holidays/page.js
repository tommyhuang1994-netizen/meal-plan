...
export default function AdminHolidaysPage() {
  ...
  function addHoliday() {
    ...
    setAll(prev => ({
      ...
      [activeGroup]: {
        ...prev[activeGroup],
        [selected]: { name: form.name || 'School Holiday', type: form.type },
      },
    }));
    setSelected(null);
    flash();
    ...
    <main style={{ background: '#FAFAFA', minHeight: '100dvh', paddingBottom: 40 }}>
      ...
      <header style={S.header}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            <h1 style={S.headerTitle}>Set Holidays</h1>
            <p style={S.headerSub}>Manage blocked dates per class group · June 2026</p>
          </div>
          {saved && (
      ...
      <div style={S.container}>
        ...
        <div style={{ background:'#FEF9C3', border:'1px solid #FDE047', borderRadius:10, padding:'9px 13px', fontSize:12, color:'#854D0E', display:'flex', gap:7, alignItems:'center' }}>
          ...
          Pre-filled from the school academic calendar. Click a school day to add or remove a blocked date. Changes apply to <strong>{activeGroup}</strong> parents only.
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:14, padding:'16px 18px', border:`1px solid ${col.border}` }}>
          <h2 style={{ fontSize:15, fontWeight:700, color:'#111827', marginBottom:14, display:'flex', alignItems:'center', gap:8 }}>
            ...
            June 2026 · {activeGroup}
          </h2>

          <div style={{ display:'grid', gridTemplateColumns:'repeat(7, 1fr)', gap:4, marginBottom:4 }}>
            {GRID_HEADERS.map(d => (
              <div key={d} style={{ textAlign:'center', fontSize:11, fontWeight:700, color: d==='Sat'||d==='Sun' ? '#D1D5DB' : '#6B7280', padding:'2px 0' }}>{d}</div>
            ))}
          </div>
                ...
                <button key={i} onClick={() => toggle(date)}
                  ...
                  {isHol && (
                    <span style={{ fontSize:8, fontWeight:600, color: isBreak ? '#D97706' : '#DC2626', lineHeight:1, textAlign:'center', padding:'0 1px', maxWidth:'100%', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                      {hInfo.type === 'break' ? 'Break' : hInfo.name.split(' ')[0]}
                    </span>
                  )}
                  {isPicked && !isHol && (
          ...
          <div style={{ display:'flex', gap:14, marginTop:12, fontSize:11, color:'#9CA3AF', flexWrap:'wrap' }}>
            <span style={{ display:'flex', alignItems:'center', gap:4 }}><span style={{ width:9, height:9, borderRadius:2, background:'#FFFBEB', border:'1px solid #FDE68A', display:'inline-block' }} /> Term break</span>
            <span style={{ display:'flex', alignItems:'center', gap:4 }}><span style={{ width:9, height:9, borderRadius:2, background:'#FFF5F5', border:'1px solid #FECACA', display:'inline-block' }} /> Public holiday</span>
            <span style={{ display:'flex', alignItems:'center', gap:4 }}>Click any school day to add or remove</span>
          </div>
        </div>
        ...
        {selected && !holidays[selected] && (
          <div style={{ background:'#fff', borderRadius:12, padding:'16px', border:`2px solid ${col.active}` }}>
            <p style={{ fontSize:14, fontWeight:700, color:'#111827', margin:'0 0 12px' }}>
              Block {dayName(selected)}, {ordinal(selected)} June 2026 for <span style={{ color:col.active }}>{activeGroup}</span>
            </p>
            ...
            <label style={S.inputLabel}>Holiday name</label>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              placeholder={form.type === 'break' ? 'e.g. Term Break' : 'e.g. Hari Raya, Sports Day...'}
        ...
        <div style={{ background:'#fff', borderRadius:12, padding:'16px', border:'1px solid #F3F4F6' }}>
          <h2 style={{ fontSize:15, fontWeight:700, color:'#111827', margin:'0 0 12px', display:'flex', alignItems:'center', gap:8 }}>
            ...
            {activeGroup} — Blocked Days
            <span style={{ fontSize:13, fontWeight:500, color:'#9CA3AF' }}>{holidayList.length} day{holidayList.length !== 1 ? 's' : ''}</span>
          </h2>

          {holidayList.length === 0 ? (
            <p style={{ fontSize:13, color:'#9CA3AF', textAlign:'center', padding:'16px 0' }}>No blocked days. Click a date on the calendar above.</p>
          ) : (
                ...
                return (
                  <div key={date} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', background: isBreak ? '#FFFBEB' : '#FFF5F5', borderRadius:9, border:`1px solid ${isBreak ? '#FDE68A' : '#FEE2E2'}` }}>
                    <div style={{ width:38, height:38, borderRadius:8, background: isBreak ? '#FEF3C7' : '#FEE2E2', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      ...
                      <span style={{ fontSize:9, color: isBreak ? '#D97706' : '#DC2626', fontWeight:600, textTransform:'uppercase' }}>Jun</span>
                    </div>
                    <div style={{ flex:1 }}>
                      <p style={{ margin:0, fontWeight:600, fontSize:14, color:'#111827' }}>{name}</p>
                      <p style={{ margin:'2px 0 0', fontSize:12, color:'#9CA3AF' }}>
                        {dayName(date)}, {ordinal(date)} June 2026
                        · <span style={{ color: isBreak ? '#D97706' : '#DC2626', fontWeight:600 }}>{isBreak ? 'Term Break' : 'Public Holiday'}</span>
                      </p>
                    </div>
                    <button onClick={() => removeHoliday(date)} title="Remove"
