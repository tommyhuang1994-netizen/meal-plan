...
const STRINGS = {
  en: {
    ...
    'adminOrders.cancelFailed': 'Could not cancel — please try again.',
    ...
    'allergy.declaredCount': '{n} with dietary requirements',
    'allergy.sectionTitle': 'Dietary requirements — {n} people',
    'allergy.vendorNote': 'Declared by parents on the order form. The wording is theirs, kept as written — a chip may be missing where the wording did not match a known category.',
    'allergy.unattributedTitle': '{n} declared requirements with no name attached',
    'allergy.unattributedBody': 'These came from responses whose name could not be read when the roster was imported. Match them to a student from the responses spreadsheet before serving.',
    ...
    'vendor.downloadPdf': 'Download PDF',
    'vendor.realData': 'Actual orders',
    'vendor.sampleData': 'Sample data',
    'vendor.colName': 'Name',
  ...
  zh: {
    ...
    'adminOrders.cancelFailed': '取消失败，请重试。',
    ...
    'allergy.declaredCount': '{n} 人有饮食要求',
    'allergy.sectionTitle': '饮食要求 — {n} 人',
    'allergy.vendorNote': '由家长在订餐表中填写。原文照录 — 若措辞未能匹配已知类别，可能没有对应标签。',
    'allergy.unattributedTitle': '{n} 项饮食要求无法对应姓名',
    'allergy.unattributedBody': '这些来自导入名单时姓名无法读取的回覆。供餐前请从回覆表中对应到具体学生。',
    ...
    'vendor.downloadPdf': '下载 PDF',
    'vendor.realData': '实际订单',
    'vendor.sampleData': '示例数据',
    'vendor.colName': '姓名',
    'vendor.colClass': '班级',
    'vendor.colMeal': '餐品',
...
// "Tuesday, 01/09/2026" — weekday plus a numeric DD/MM/YYYY date, for billing
// tables where a column of dates has to line up and be easy to match against
// an invoice. Same numeric part in both languages.
export function fmtDateNumeric(lang, monthKey, dayNum) {
  const { year, monthIdx, d } = partsOf(monthKey, dayNum);
  const dd = String(dayNum).padStart(2, '0');
  const mm = String(monthIdx + 1).padStart(2, '0');
  return `${WD_FULL[lang === 'zh' ? 'zh' : 'en'][d]}, ${dd}/${mm}/${year}`;
}
