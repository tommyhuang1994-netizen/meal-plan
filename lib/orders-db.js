...
export function cycleKey(monthKey) {
  const [year, month] = monthKey.split('-').map(Number);
  return { year, month };
}

export const isoOf = (date) => date.toISOString().slice(0, 10);
...
/// An empty or omitted name list means every student — which is what the admin
/// view needs, and what the parent view must never pass.
export async function studentOrders(studentNames) {
  const scoped = Array.isArray(studentNames) && studentNames.length > 0;
  const orders = await prisma.order.findMany({
    where: {
      ...(scoped ? { student: { name: { in: studentNames } } } : {}),
      status: { not: 'CANCELLED' },
    },
    include: {
      student: true,
