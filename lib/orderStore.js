...
import { isDateAvailable } from './schoolCalendar.js';
import { REAL_ORDERS, REAL_DATES } from './realOrders.js';
...
// The month lib/schoolCalendar.js describes. Term breaks differ per class group
// — Homeschool is away 4, 14, 15, 17, 18 September and Plus 4, 7–11 — so a
// date that serves one group does not serve them all.
const CALENDAR_MONTH = '2026-09';

/// Does this class group actually attend on this date? A day the group is away
/// must never carry a charge: it would bill a parent, and the school, for a
/// child who was not there. Months with no published group calendar (August)
/// fall back to the month-wide closures above.
export function groupServes(monthKey, day, classGroup) {
  if (monthKey !== CALENDAR_MONTH) return !closedReason(monthKey, day);
  return isDateAvailable(classGroup, day);
}
...
function sampleOrdersFor(monthKey, day) {
  ...
  for (const student of ROSTER) {
    // Away on term break — this group is not served today at all.
    if (!groupServes(monthKey, day, student.dept)) continue;
...
// ── Real orders taken from the school's vendor name lists ────────────────────
// These are the record of what was actually served, so where a date is on file
// it replaces the generated samples entirely. Allergies come from the roster,
// since the service sheet does not carry them.

function realOrdersFor(iso) {
  const byName = new Map(ROSTER.map(s => [s.name, s]));
  return (REAL_ORDERS[iso] ?? []).map(o => {
    const s = byName.get(o.name);
    return {
      id: `real-${iso}-${o.name}`,
      date: iso,
      name: o.name,
      dept: o.dept ?? s?.dept ?? null,
      year: o.year ?? s?.year ?? null,
      allergies: s?.allergies ?? [],
      note: s?.note ?? '',
      breakfast: o.breakfast ?? null,
      lunch: o.lunch ?? null,
      brunch: o.brunch ?? null,
      source: 'real',
    };
  });
}

/// Is this date backed by a real name list rather than generated samples?
export function isRealDate(monthKey, day) {
  const m = MONTHS.find(x => x.key === monthKey);
  return !!m && REAL_DATES.has(isoDate(m.year, m.month, day));
}
...
export function getOrdersForDate(monthKey, day) {
  ...
  // Fail closed: a row on a date its group does not attend is dropped whatever
  // its source, so a stale submission can never reach an invoice either.
  const submitted = readSubmitted().filter(
    r => r.date === iso && groupServes(monthKey, day, r.dept)
  );
  const names = new Set(submitted.map(r => r.name));
  // A real name list wins over generated samples; a parent's own order still
  // wins over both, so an order placed in the portal shows what they chose.
  const base = REAL_DATES.has(iso) ? realOrdersFor(iso) : sampleOrdersFor(monthKey, day);
  const rest = base.filter(r => !names.has(r.name) && groupServes(monthKey, day, r.dept));
  return [...submitted, ...rest].sort((a, b) => a.name.localeCompare(b.name));
}
